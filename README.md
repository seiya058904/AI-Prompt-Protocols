...
- **Multiple-screenshot relationships.** When several screenshots are supplied, their relationship is classified first (same page different viewport / different state / different pages / unrelated); viewport variants feed responsive evidence, state variants feed interaction evidence, and different pages are never merged into one imagined interface.
...
- **Outgoing-commit audit before push.** Before any push, the exact outgoing range (`git log --oneline <upstream>..HEAD`, plus `git diff --stat` when useful) is inspected; a normal push may publish pre-existing local commits, so unrelated, pre-existing, or uncertain commits are never pushed just to make the repo synced — history is not rewritten to remove them, and the situation is surfaced for user decision.
