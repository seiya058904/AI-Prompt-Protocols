...
- **安全且合理的假设优先于提问。** 只有当歧义或缺失信息可能实质性地改变结果、导致数据丢失、造成不可逆结局或需要用户真实选择时，才提出澄清问题；否则选择最安全且合理的假设并继续。
...
```text
* 只有在歧义或缺失信息可能实质性地改变结果、导致数据丢失、造成不可逆结局或需要用户真实选择时，才提出澄清问题。否则选择最安全且合理的假设，在相关时说明该假设，然后继续。
...
- **同样的核心规则。** 行为契约——安全且合理的假设、最小改动、删除安全、外部副作用闸门、验证——与 Codex 版本完全一致。
...
- **多截图关系判定。** 提供多张截图时，先判断其关系（同一页面不同视口 / 不同状态 / 不同页面 / 互不相关）；视口变体作为响应式证据、状态变体作为交互证据，不同页面绝不合并成一个想象中的界面。
...
- **push 前审计 outgoing commits。** 任何 push 前先检查精确的 outgoing 范围（`git log --oneline <upstream>..HEAD`，需要时再加 `git diff --stat`）；普通 push 可能发布 pre-existing local commits，因此无关、预存或不确定的 commits 绝不为“让仓库同步”而推送——也不通过改写历史移除它们，而是保留现状并呈现给用户决策。
